...
> {
  const data =
    await api<ConversationsResponse>(
      '/api/conversations',
      {
        authenticated: true,
      },
...
export async function createDirectConversation(
  ...
  const data =
    await api<ConversationResponse>(
      `/api/conversations/direct/${participantId}`,
      {
        method: 'POST',
        authenticated: true,
      },
...
export async function acceptContactRequest(
  ...
  return api<{
    contact: ContactRequest;
  }>(`/api/contacts/requests/${contactId}/accept`, {
    method: 'POST',
    authenticated: true,
  });
}
...
export async function rejectContactRequest(
  ...
  const data = await api<{
    ...
    `/api/contacts/requests/${contactId}/reject`,
    {
      method: 'POST',
      authenticated: true,
    },
